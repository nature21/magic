...
def normalize_vector(v: np.ndarray) -> np.ndarray:
    """Normalize a vector."""
    return v / (np.linalg.norm(v) + _FLOAT_EPS)
...
def quat2matrix(quat, homogeneous=False):
    """Convert a quaternion to a rotation matrix.The quaternion is represented as a 4-element array in the order of (x, y, z, w),
    which follows the convention of pybullet.
    ...
    assert quat.shape[-1] == 4, "Invalid shape quaternion {}".format(quat)
...
def quat_diff_in_axis_angle(quat1, quat2):
    """Compute the difference between two quaternions (q1 - q2) in axis-angle representation.

    Note that since the difference is computed as q1 - q2, for conventional usage of computing
    the difference between the source and target, the order should be quat_diff_in_axis_angle(target, source).
    """
    quat1 = np.asarray(quat1)
    quat2 = np.asarray(quat2)
    assert quat1.shape == quat2.shape
    assert quat1.shape[-1] == 4
    assert quat2.shape[-1] == 4

    quat_diff = quat_mul(quat1, quat_conjugate(quat2))
    quat_diff = quat_diff[..., [3, 0, 1, 2]]  # wxyz -> xyzw
...
def quat_mul_wxyz(q0, q1, *args):
    if len(args) > 0:
        return quat_mul_wxyz(q0, quat_mul_wxyz(q1, *args))

    q0_xyzw = wxyz2xyzw(q0)
    q1_xyzw = wxyz2xyzw(q1)
    q_xyzw = quat_mul(q0_xyzw, q1_xyzw)
    return xyzw2wxyz(q_xyzw)
...
def quat_conjugate_wxyz(q_wxyz):
    q_xyzw = wxyz2xyzw(q_wxyz)
    q_xyzw = quat_conjugate(q_xyzw)
    return xyzw2wxyz(q_xyzw)
