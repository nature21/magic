...
def load_model(config_path="Panoptic/odise_label_coco_50e.py", seed=42, diffusion_ver="v1-3", image_size=1024,
               num_timesteps=0, block_indices=(2, 5, 8, 11), decoder_only=True, encoder_only=False,
               resblock_only=False):
...
def process_features_and_mask(model, aug, image, category=None, input_text=None, mask=True, pca=False, raw=False):
    ...
    features_gether_s4_s5 = torch.cat(
        [features['s4'], F.interpolate(features['s5'], size=(features['s4'].shape[-2:]), mode='bilinear')], dim=1)
