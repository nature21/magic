...
def resize(img: Image, target_res, resize=True, to_pil=True, edge=False):
...
def draw_correspondences_lines(points1: List[Tuple[float, float]], points2: List[Tuple[float, float]], 
    ...
    :return: a figure of image_scene with marked points and lines between them showing correspondence.
    """
...
def animate_image_transfer(image1, image2, mapping, output_path):
    ...
    # # Load your two image_scene
    # image1 = Image.open(image1_path)
    # image2 = Image.open(image2_path)

    # Ensure the two image_scene are the same size
    assert image1.size == image2.size, "Images must be the same size."
    rec_size = 2
    # Convert the image_scene into numpy arrays
    image1_array = np.array(image1)
    image2_array = np.array(image2)

    # Retrieve the width and height of the image_scene
    height, width, _ = image1_array.shape

    # Assume we have a mapping list
    mapping = mapping.cpu().numpy()

    # We add a column of white pixels between the two image_scene
    gap = width // 10

    # Create a canvas with a width that is the sum of the widths of the two image_scene and the gap.
    # The height is the same as the height of the image_scene.
